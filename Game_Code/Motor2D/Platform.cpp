...
//Constructors ----------------------------------
Platform::Platform(const SDL_Rect& texture_rect, PLATFORM_TYPE TYPE): type(TYPE)
{
...
{
	body = App->physics->CreateRectangle(0,0, texture_rect.w, texture_rect.h, collision_type::PLATFORM, BODY_TYPE::platform);
...
void Platform::GenerateTextureFromRect(const SDL_Rect& texture_rect)
{
	...
	texture = SDL_CreateTexture(App->render->renderer, SDL_PIXELFORMAT_ABGR8888, SDL_TEXTUREACCESS_TARGET, texture_rect.w, texture_rect.h);
	...
	int max_mid_length = texture_rect.w - right_texture.w;
	//Blit mid texture to cover all the texture width
	//Set limited viewport to give free texture space to blit right corner
	App->render->SetViewPort({ 0,0,max_mid_length ,texture_rect.h });

	while (w_lenght < max_mid_length)
	{
		App->render->Blit(spritesheet, w_lenght,0, &mid_texture);
		w_lenght += mid_texture.w;
	}
	//Reset viewport to the new texture size
	App->render->SetViewPort(texture_rect);
	
	//Blit side textures to close platform corners
	App->render->Blit(spritesheet, 0, 0, &left_texture);
	App->render->Blit(spritesheet, texture_rect.w - right_texture.w, 0, &right_texture);
